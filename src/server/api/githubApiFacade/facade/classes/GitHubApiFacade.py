...
class GitHubApiFacade:
    ...
    def __get_followers(self): return self.__fetch_feature(
        API_FEATURES.FOLLOWERS.value)
    ...
    def __get_total_stars(self):
        repos_data = self.__fetch_feature_route(
            API_FEATURES.STARGAZERS_COUNT.value)
        total_stars = 0
    ...
    def __get_total_contributions(self):
        issues_data = self.__fetch_feature_route(
            API_FEATURES.CONTRIBUTIONS.value)
        return issues_data['total_count']
    ...
    def get_feature(self, feature):
        ...
        if(feature == API_FEATURES.STARGAZERS_COUNT.value):
            return self.__get_total_stars()
        if(feature == API_FEATURES.FORKS_COUNT.value):
            return self.__get_total_forks()
        if(feature == API_FEATURES.COMMITS.value):
            return self.__get_total_commits()
        if(feature == API_FEATURES.ISSUES.value):
            return self.__get_total_issues()
        if(feature == API_FEATURES.CONTRIBUTIONS.value):
            return self.__get_total_contributions()
        if(feature == API_FEATURES.ORGS.value):
            return self.__get_total_orgs()
    ...
    def fetch_all_feature(self):
        ...
        for feature in API_FEATURES:
            fetch_result[API_OUTPUTS[feature.name].value] = self.get_feature(
                feature.value)
        return fetch_result
